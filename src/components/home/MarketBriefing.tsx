...
function Pill({
  ...
  return (
    <span
      className={cx(
        'inline-flex items-center gap-1.5 rounded-full px-3 py-2 text-[12px] leading-none backdrop-blur-xl',
        cls,
      )}
    >
      {label}
...
function thesisFor(slug: string): Briefing {
  ...
  if (slug === 'london')
    return {
      tone: 'tightening',
      thesis: 'Capital is cautious and concentrated. Prime stability while secondary softness persists.',
...
function Meter({
  ...
  return (
    <div className={cx('min-w-[180px] flex-1 rounded-[28px] px-5 py-4', CARD)}>
...
function MiniBar({
  ...
  const bar =
    tone === 'violet'
      ? 'bg-[linear-gradient(90deg,rgba(139,92,246,0.55),rgba(231,201,130,0.28))]'
      : tone === 'good'
        ? 'bg-[linear-gradient(90deg,rgba(16,185,129,0.55),rgba(11,12,16,0.26))]'
        : 'bg-[rgba(11,12,16,0.45)]';

  return (
    <div className={cx('rounded-[28px] px-5 py-4', CARD)}>
      ...
      <div className="mt-3 h-2 w-full rounded-full bg-[rgba(11,12,16,0.08)]">
        <div className={cx('h-2 rounded-full', bar)} style={{ width: barPct(pct) }} />
      </div>
    </div>
  );
}
      ...
      <div className="mx-auto w-full max-w-[1560px] px-5 sm:px-8 lg:px-10">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="min-w-0">
            ...
            <h2 className="mt-3 text-balance text-[26px] font-semibold tracking-[-0.02em] text-[color:var(--ink)] sm:text-[34px]">
              Decision-grade signal, not portal theatre
            </h2>

            <p className="mt-2 max-w-3xl text-[14px] leading-relaxed text-[color:var(--ink-2)]">
              A compressed view of liquidity, price reality and proof density so you move with discipline.
            </p>
          </div>
        ...
        {/* Main slab */}
        <div className={cx('mt-6 relative overflow-hidden rounded-[34px] p-6 sm:p-8 lg:p-10', CARD)}>
          ...
          <div className="relative">
            ...
            <div
              className={cx(
                'mt-5 rounded-[28px] px-5 py-4 text-[12px]',
                'bg-white/72',
                RING,
            ...
            >
              <span
                className={cx(
                  'mr-2 inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[11px]',
                  'bg-white/78',
                  RING,
                  'text-[color:var(--ink-2)]',
                )}
              ...
              <span className="text-[color:var(--ink-3)]">
                Price reductions rose in one market, proof density improved in another, and one moved into scarcity-led
                bidding.
              </span>
            </div>

            <div className="mt-5 grid gap-3 lg:grid-cols-3">
              <div className={cx('rounded-[28px] px-5 py-4', 'bg-white/72', RING)}>
                <div className="text-[10px] font-semibold tracking-[0.28em] text-[color:var(--ink-3)]">PORTAL OUTPUT</div>
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-3">
          {briefs.map(({ city: c, b }) => {
            ...
            const toneForLiquidity: 'neutral' | 'violet' | 'good' | 'warn' =
              b.liquidityHeat >= 70 ? 'violet' : b.liquidityHeat >= 55 ? 'neutral' : 'warn';

            const toneForPressure: 'neutral' | 'violet' | 'good' | 'warn' =
              b.buyerPressure >= 70 ? 'violet' : b.buyerPressure >= 55 ? 'neutral' : 'warn';
              ...
              >
                ...
                <div className="relative">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span
                      className={cx(
                        'inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[11px] backdrop-blur-xl',
                        t.pill,
                      )}
                    >
                  ...
                  <div className="mt-5 grid gap-2">
                    <MiniBar label="Liquidity heat" value={scoreLabel(b.liquidityHeat)} pct={b.liquidityHeat} tone={toneForLiquidity} />
                    <MiniBar label="Buyer pressure" value={scoreLabel(b.buyerPressure)} pct={b.buyerPressure} tone={toneForPressure} />
