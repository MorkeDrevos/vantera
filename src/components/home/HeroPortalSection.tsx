...
// Deterministic, production-looking placeholder signals (until wired to DB).
// Same city slug -> same values, so it feels stable not random.
...
function modelSignals(city: HeroCity) {
  const a = hash01(city.slug + '|liq');
  const b = hash01(city.slug + '|risk');
  const c = hash01(city.slug + '|sup');
  const d = hash01(city.slug + '|sqm');

  // Liquidity: 48..92
  const liquidity = Math.round(48 + a * 44);

  // Risk: 18..72 (luxury markets tend to be “mid” not extreme)
  const risk = Math.round(18 + b * 54);

  // Verified supply: 120..980 (looks plausible for flagship inventories)
  const verifiedSupply = Math.round(120 + c * 860);

  // Median €/sqm: 7.5k .. 32.5k (luxury range)
  const medianEurSqm = Math.round(7_500 + d * 25_000);
...
function MetaPill({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-flex items-center gap-2 border border-[rgba(10,10,12,0.12)] bg-white/92 px-3 py-1.5 text-[11px] font-semibold tracking-[0.26em] text-[color:var(--ink-2)] backdrop-blur-[10px]">
      {children}
    </span>
  );
}
...
export default function HeroPortalSection({
  ...
  const resolvedSignals = useMemo(() => {
    if (!active) return { modelled: false, s: null as null | NonNullable<HeroCity['signals']> };
    const hasAny =
      active.signals &&
      (typeof active.signals.liquidity === 'number' ||
        typeof active.signals.risk === 'number' ||
        typeof active.signals.verifiedSupply === 'number' ||
        typeof active.signals.medianEurSqm === 'number');

    if (hasAny) return { modelled: false, s: active.signals! };
    return { modelled: true, s: modelSignals(active) };
  }, [active]);

  const overlaySignals = useMemo(() => {
    if (!resolvedSignals.s) return [];

    const s = resolvedSignals.s;
          ...
          <div className="pb-10 pt-10 sm:pb-12 sm:pt-12">
            <div className="max-w-[1040px]">
              <div className="flex flex-wrap items-center gap-2">
                <MetaPill>MARKETPLACE</MetaPill>
                <MetaPill>TRUTH LAYER</MetaPill>
                <MetaPill>EDITORIAL</MetaPill>
              </div>
              ...
              {/* HERO SEARCH - bigger, stable (no jump on focus) */}
              <div className="mt-7">
                <div
                  className={cx(
                    'w-full max-w-[1320px]',
                    // lock layout so focus styles cannot shift things
                    'rounded-[28px]',
                    // If OmniSearch uses focus rings/shadows internally, these overrides keep it stable
                    '[&_*]:outline-none',
                    '[&_*]:ring-0',
                    '[&_*]:ring-offset-0',
                    // Increase input + button sizing (safe generic selectors)
                    '[&_input]:text-[18px]',
                    '[&_input]:leading-none',
                    ...
                    '[&_button]:h-[54px]',
                    '[&_button]:px-5',
                    '[&_[role=button]]:h-[54px]',
                    // avoid “jump” from focus-within shadow changes
          ...
          <div className="pb-12 sm:pb-14">
            <div className="relative">
              ...
              <div className="flex items-end justify-between gap-6">
                ...
                {/* Controls - premium, but NO DOT RAIL (img1 removed) */}
