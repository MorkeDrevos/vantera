...
function ModeBanner({ mode }: { mode: TruthCardMode }) {
  if (mode === 'locked') {
    return (
...
export default function TruthCard({
  ...
  const demandTone = data.demandPressure === 'high' ? 'good' : data.demandPressure === 'low' ? 'warn' : 'neutral';
      ...
      <div className="relative">
        ...
        {isLocked ? (
          <div className="mt-5 rounded-[26px] border border-white/10 bg-black/30 p-5">
            <div className="text-sm text-zinc-300">Listings are locked until verified supply exists.</div>
